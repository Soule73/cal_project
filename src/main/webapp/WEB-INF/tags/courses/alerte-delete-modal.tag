...
<div x-data="deleteCourseForm()" x-init="init()" x-cloak>

    <div x-show="modalOpen" x-transition="" class="fixed left-0 top-0 z-999999 flex h-full min-h-screen w-full items-center justify-center bg-black/90 px-4 py-5 text-red-600">
        <div @click.outside="modalOpen = false" class="w-full max-w-142.5 rounded-lg bg-white px-8 py-12 text-center dark:bg-boxdark md:px-17.5 md:py-15">
            ...
            <h3  x-show="!successMessage" x-text="modalTitle" class="mt-5.5 capitalize pb-2 text-xl font-bold text-black dark:text-white sm:text-2xl">
              
            </h3>
            <p  x-show="!successMessage" x-text="modalDesc" class="mb-10 font-medium">
            ...
            <div  x-show="!successMessage" class="-mx-3 flex flex-wrap gap-y-4">
                ...
                <div class="w-full px-3 2xsm:w-1/2">
                    <button @click="submitForm" x-text="submitBtnText" class="block w-full rounded border border-meta-1 bg-meta-1 p-3 text-center font-medium text-white transition hover:bg-opacity-90">
            ...
            >
                <div
                        class="flex w-max p-4 w-full  items-center justify-center rounded-full bg-[#34D399]"
                >
                    <svg
                            width="56"
                            height="50"
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('deleteCourseForm', () => ({
            ...
            init(){
                window.deleteCourseForm = this.setDelteForm.bind(this);
            },

            submitForm() {
                const data = JSON.stringify(this.deltetForm);
                ...
                fetch('<%=Routes.COURSE_FORM%>', {
                    ...
                    .then(data => {
                        if (data.errors) {
                            ...
                            this.resetDeleteForm();
                            this.successMessage = "Le cours a été supprimée avec succès!";
