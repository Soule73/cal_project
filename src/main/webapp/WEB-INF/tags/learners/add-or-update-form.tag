    ...
    <div x-show="modalOpen" x-transition class="overflow-y-auto md:max-h-[80vh] fixed left-0 top-0 z-999999 flex h-full min-h-screen w-full items-center justify-center bg-black/90 px-4 py-5">
        <div @click.outside="closeModal" class="w-full max-w-142.5 rounded-lg bg-white px-8 py-12 text-center dark:bg-boxdark md:px-17.5 md:py-15">
            ...
            <form x-show="!successMessage" @submit.prevent="submitForm">
                <tg:x-input-form label="Prénom" name="firstname" placeholder="Entrer le prénom de l'apprenant" isRequired="true" xmodel="form.firstname" />
                <tg:x-input-form label="Nom" name="lastname" placeholder="Entrer le nom de l'apprenant" isRequired="true" xmodel="form.lastname" />
                <tg:x-input-form label="E-mail" name="email" type="email" placeholder="Entrer l'e-mail de l'apprenant" isRequired="true" xmodel="form.email" />
                <div class="mb-4 px-2">
                    <label for="levelId" class="mb-3 block text-sm font-medium text-black dark:text-white">Niveau</label>
                    <select id="levelId" name="levelId" x-model="form.levelId" class="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent px-5 py-3 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary">
                        <option value="" class="text-body">Sélectionner un niveau</option>
                ...
                <div class="mb-4 px-2">
                    <label for="languageId" class="mb-3 block text-sm font-medium text-black dark:text-white">Langue</label>
                    <select id="languageId" name="languageId" x-model="form.languageId" class="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent px-5 py-3 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary">
                        <option value="" class="text-body">Sélectionner une langue</option>
                ...
                <div class="mb-4 px-2">
                    <label for="password" class="mb-3 block text-sm font-medium text-black dark:text-white">Mot de passe</label>
                    <div class="flex items-center">
                        <input id="password" name="password" type="text" x-model="form.password" class="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent px-5 py-3 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary" placeholder="Entrer ou générer un mot de passe">

                        <button type="button" @click="generatePassword" class="ml-2 rounded-md px-4 py-2 text-sm font-medium border border-primary bg-primary  text-center font-medium text-white transition hover:bg-opacity-90">Générer</button>
                    </div>
                    <span x-text="errors.password" class="text-red-600"></span>
                </div>
