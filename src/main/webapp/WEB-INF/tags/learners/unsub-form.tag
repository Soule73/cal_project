...
<div x-data="deleteSubscriptionForm()" x-init="init()" x-cloak>
    <div x-show="modalOpen" x-transition="" class="fixed left-0 top-0 z-999999 flex h-full min-h-screen w-full items-center justify-center bg-black/90 px-4 py-5" style="display: none;">
        <div @click.outside="modalOpen = false" class="w-full max-w-142.5 rounded-lg bg-white px-8 py-12 text-center dark:bg-boxdark md:px-17.5 md:py-15">
            ...
            <h3 x-show="!successMessage" style="text-transform: capitalize;" class="mt-5.5 capitalize pb-2 text-xl font-bold text-black dark:text-white sm:text-2xl">
                Êtes-vous sûr de désabonner l'apprenant?
            </h3>
            ...
            <div x-show="!successMessage" class="-mx-3 flex flex-wrap gap-y-4">
                ...
                <div class="w-full px-3 2xsm:w-1/2">
                    <button @click="submitForm" class="block w-full rounded border border-meta-1 bg-meta-1 p-3 text-center font-medium text-white transition hover:bg-opacity-90">
                        Désabonner
                    </button>
                </div>
            </div>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('deleteSubscriptionForm', () => ({
            modalOpen: false,
            modalDesc: "",
            deleteForm: {
                learnerId: null,
            ...
            setDeleteForm(data) {
                this.deleteForm.learnerId = data.learnerId;
                this.deleteForm.subscriptionId =data.subscriptionId;
                this.modalDesc = "Abonnement : " + data.subscriptionName;
                this.modalOpen = true;
            ...
            init() {
                window.unSubcrForm = this.setDeleteForm.bind(this);
            },
                ...
                fetch('<%=Routes.LEARN_SUBCRIPTIONS%>', {
                    ...
                    .then(data => {
                        if (data.errors) {
                            ...
                            this.resetDeleteForm();
                            this.successMessage = "L'apprenant a été désabonné avec succès !";
