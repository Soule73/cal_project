...
<div x-data="deleteSubscriptionForm()" x-init="init()" x-cloak>
    <div x-show="modalOpen" x-transition="" class="fixed left-0 top-0 z-999999 flex h-full min-h-screen w-full items-center justify-center bg-black/90 px-4 py-5" style="display: none;">
        <div @click.outside="modalOpen = false" class="w-full max-w-142.5 rounded-lg bg-white px-8 py-12 text-center dark:bg-boxdark md:px-17.5 md:py-15">
            ...
            <h3 x-show="!successMessage" style="text-transform: capitalize;" x-text="modalTitle" class="mt-5.5 capitalize pb-2 text-xl font-bold text-black dark:text-white sm:text-2xl"></h3>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('deleteSubscriptionForm', () => ({
            modalTitle: "Êtes-vous sûr de vouloir supprimer cet abonnement ?",
            ...
            setDeleteForm(subscription) {
                this.deleteForm = { ...subscription };
                this.modalDesc = "L'abonnement : "+this.deleteForm.name;
                this.modalOpen = true;
            },

            resetDeleteForm() {
                this.modalTitle = "Êtes-vous sûr de vouloir supprimer cet abonnement ?";
            ...
            init() {
                window.deleteSubscriptionForm = this.setDeleteForm.bind(this);
            },
                ...
                fetch('<%=Routes.SUBCRIPTION_FORM%>', {
                    ...
                    .then(data => {
                        if (data.errors) {
                            ...
                            this.resetDeleteForm();
                            this.successMessage = "L'abonnement a été supprimé avec succès!";
