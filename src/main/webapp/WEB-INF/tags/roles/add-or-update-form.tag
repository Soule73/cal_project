...
<div x-data="roleForm()" x-init="init()" x-cloak>
    ...
    <div x-show="modalOpen" x-transition="" class="fixed left-0 top-0 z-999999 flex h-full min-h-screen w-full items-center justify-center bg-black/90 px-4 py-5" x-cloak>
        <div @click.outside="closeModal" class="w-full max-w-142.5 rounded-lg bg-white px-8 py-12 text-center dark:bg-boxdark md:px-17.5 md:py-15">
            ...
            <form x-show="!successMessage" @submit.prevent="submitForm">
                ...
                <tg:x-input-form label="Nom de le role" name="name" placeholder="Entrer le nom de le role" isRequired="true" xmodel="form.name" />
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('roleForm', () => ({
            ...
            setForm(role) {
                this.form = { ...role };
                this.formTitle = this.form.id ? "Modifier une role" : "Ajouter un role";
                this.submitBtnText = this.form.id ? "Sauvegarder" : "Ajouter";
                this.modalOpen = true;
            },

            resetForm() {
                this.formTitle = "Ajouter un role";
            ...
            init() {
                window.roleForm = this.setForm.bind(this);
            },
                ...
                fetch('<%=Routes.ROLE_FORM%>', {
                    ...
                    .then(data => {
                        if (data.errors) {
                            ...
                            this.successMessage = this.form.id ? "Le role a été mise à jour avec succès!" : "Le role a été ajoutée avec succès!";
                            // Appeler la méthode globale pour rafraîchir les roles
                            window.refreshRoles();
                        }
                    })
